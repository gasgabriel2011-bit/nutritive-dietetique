...
export default function PlanBoissons({ drinks, backTo }) {
    ...
    <div className="pt-20 pb-20 bg-background">
      <div className="max-w-4xl mx-auto px-4 sm:px-6">
        <AnimatedSection>
          ...
          <p className="text-sm text-muted-foreground mb-8">Cliquez pour les détails.</p>
        </AnimatedSection>

        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {drinks.map((d, i) => (
            <AnimatedSection key={d.name} delay={i * 0.05}>
              <motion.div whileHover={{ y: -3 }} onClick={() => setSelected(d.name)}
                className={`rounded-3xl p-5 border cursor-pointer transition-all duration-300 hover:shadow-xl group ${
                  ...
                  d.ok === false ? 'bg-card border-destructive/20 opacity-80' :
                  'bg-card border-border/50 hover:border-primary/30'
                }`}>
                <div className="flex items-start gap-3">
                  <span className="text-3xl">{d.icon}</span>
                  <div>
                    <h3 className={`font-semibold text-sm mb-1 group-hover:text-primary transition-colors ${d.ok === false ? 'line-through text-muted-foreground' : ''}`}>{d.name}</h3>
                    <p className="text-xs text-muted-foreground">{d.rec}</p>
                  </div>
                </div>
                <p className="mt-3 text-xs font-medium text-primary">Détails →</p>
              </motion.div>
            </AnimatedSection>
          ))}
        </div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ opacity: 0, y: 30, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }}
              ...
              <div className="p-6">
                ...
                <p className="text-sm font-semibold text-primary mb-3">{drink.rec}</p>
