...
import { REEQUILIBRAGE_DAY_PLANS } from '../../lib/reequilibrageData';
import { getOptimizedImageUrl } from '@/lib/imageOptimization';

export default function ReequDays() {
  const [selected, setSelected] = useState('a');
  const plan = REEQUILIBRAGE_DAY_PLANS.find(d => d.id === selected);
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6">
        <AnimatedSection>
          ...
          <p className="text-muted-foreground mb-8">
            4 journées types pour varier selon vos envies et votre mode de vie.
          </p>
        </AnimatedSection>

        <div className="flex gap-2 overflow-x-auto pb-2 mb-8">
          {REEQUILIBRAGE_DAY_PLANS.map(d => (
            <button
              key={d.id}
          ...
          <motion.div key={plan.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
            ...
            <div className="mt-8 bg-primary/5 rounded-3xl p-6 border border-primary/10 text-center">
              <p className="text-sm text-muted-foreground">
                Ces journées sont des <strong>exemples indicatifs</strong>. Adaptez selon vos goûts, votre appétit et votre rythme de vie.
              </p>
            </div>
          </motion.div>
        )}
