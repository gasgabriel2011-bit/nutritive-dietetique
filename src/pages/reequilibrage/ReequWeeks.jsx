...
import { REEQUILIBRAGE_WEEKS } from '../../lib/reequilibrageData';

export default function ReequWeeks() {
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6">
        <AnimatedSection>
          ...
          <p className="text-muted-foreground mb-10">
            12 semaines progressives. Cliquez sur chaque semaine pour tous les détails.
          </p>
        </AnimatedSection>

        <div className="space-y-4">
          {REEQUILIBRAGE_WEEKS.map((week, i) => (
                  ...
                  {expandedWeek === week.num && (
                    ...
                    >
                      <div className="px-6 pb-6 border-t border-border/40 space-y-6">
                        ...
                        <div>
                          <h3 className="text-xs font-bold text-secondary uppercase tracking-wider mb-3">✅ Check-list</h3>
                          <div className="flex flex-wrap gap-2">
                            {week.checklist.map((c, j) => (
                              <span key={j} className="px-3 py-1.5 rounded-full bg-secondary/10 text-secondary text-xs font-medium">{c}</span>
                            ))}
                          </div>
                        </div>
