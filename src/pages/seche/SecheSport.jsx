...
import { SPORT_TIPS } from '../../lib/secheData';

export default function SecheSport() {
  const [selected, setSelected] = useState(null);
  const tip = SPORT_TIPS.find(t => t.title === selected);
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6">
        <AnimatedSection>
          ...
          <p className="text-muted-foreground mb-10">
            Cliquez sur chaque carte pour des conseils approfondis.
          </p>
        </AnimatedSection>
            ...
            <AnimatedSection key={tip.title} delay={i * 0.07}>
              ...
              >
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <h2 className="font-display text-lg font-semibold text-foreground group-hover:text-primary transition-colors mb-1">
                      {tip.title}
                    </h2>
            ...
            >
              <div className="p-6 sm:p-8">
                <div className="flex items-start justify-between mb-5">
                  ...
                  <button
                    onClick={() => setSelected(null)}
                    className="w-9 h-9 rounded-full bg-muted flex items-center justify-center hover:bg-muted/70 transition-colors shrink-0 ml-4"
                  >
                    <X className="w-4 h-4" />
                  </button>
                </div>
