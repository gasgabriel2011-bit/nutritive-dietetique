...
import { SHOPPING_LIST } from '../../lib/secheData';

const STORAGE_KEY = 'nutrivie_seche_shopping';

export default function SecheShopping() {
  ...
  const totalItems = SHOPPING_LIST.reduce((acc, c) => acc + c.items.length, 0);
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6">
        <AnimatedSection>
          ...
          <div className="flex items-center justify-between flex-wrap gap-4 mb-3">
            ...
            <button onClick={reset} className="text-xs text-muted-foreground hover:text-destructive transition-colors underline">
              Tout décocher
            </button>
          </div>
          <p className="text-muted-foreground mb-3">
            Liste complète pour une semaine du programme. Cochez les articles au fur et à mesure.
          </p>
          {/* Progress */}
                  ...
                  {cat.items.map((item, ii) => (
                    <li key={ii}>
                      ...
                      >
                        ...
                        <span className={`text-sm ${isChecked(cat.cat, item) ? 'line-through text-muted-foreground' : 'text-foreground'}`}>
                          {item}
                        </span>
                      </button>
                    </li>
        ...
        <div className="mt-10 bg-primary/5 rounded-3xl p-6 border border-primary/10 text-center">
          <p className="text-sm text-muted-foreground">
            💡 <strong className="text-foreground">Astuce :</strong> Faites vos courses le <strong>samedi</strong> et le meal prep le <strong>dimanche</strong> pour aborder la semaine sereinement.
          </p>
        </div>
      </div>
    </div>
