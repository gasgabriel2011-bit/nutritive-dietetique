...
import { PRISE_MASSE_SPORT } from '../../lib/priseMasseData';

export default function PriseMasseSport() {
  const [selected, setSelected] = useState(null);
  const tip = PRISE_MASSE_SPORT.find(t => t.title === selected);
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6">
        <AnimatedSection>
          ...
          <h1 className="font-display text-4xl sm:text-5xl font-light text-foreground mb-3">
            Conseils <span className="italic font-semibold text-secondary">sport & récupération</span>
          </h1>
          <p className="text-sm text-muted-foreground mb-8">Cliquez sur chaque conseil pour les détails complets.</p>
        </AnimatedSection>

        <div className="grid sm:grid-cols-2 gap-5">
          {PRISE_MASSE_SPORT.map((tip, i) => (
            <AnimatedSection key={tip.title} delay={i * 0.06}>
              ...
              >
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <h3 className="font-display text-lg font-semibold group-hover:text-secondary transition-colors mb-1">{tip.title}</h3>
                    <p className="text-sm text-muted-foreground leading-relaxed">{tip.desc}</p>
                  </div>
                </div>
                <p className="mt-3 text-secondary text-xs font-medium">En savoir plus →</p>
              </motion.div>
            </AnimatedSection>
          ))}
        </div>
            ...
            >
              <div className="p-6 sm:p-8">
                ...
                <div className="bg-muted/50 rounded-2xl p-4 mb-5">
                  <h3 className="text-xs font-bold uppercase tracking-wider text-secondary mb-3">Points clés</h3>
